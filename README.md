...
**Phase 22 (current):** gym-owned branches. Owner/admin create, rename, and archive locations. The platform key is no longer required to open a second site. A gym must keep one active branch; archive is blocked while active desk staff are assigned there.
...
| POST | `/api/v1/branches` | Owner, admin (`name`) |
| GET | `/api/v1/branches/:id` | Owner, admin |
| PATCH | `/api/v1/branches/:id` | Owner, admin (`name`, `status`) |
...
| GET | `/api/v1/members` | Any staff (`search`, `code`, `status`, `homeBranchId`, `sort`, `page`, `limit`) |
...
| GET | `/api/v1/subscriptions` | Any staff (`memberId`, `planId`, `soldByStaffId`, `cancelledByStaffId`, `status`, `unpaid`, `inProgress`, `completedUnrenewed`, `expired`, `sort`, `page`, `limit`) |
...
Subscribing snapshots the sold terms (`durationDays`, `sessionCount`, `sessionsRemaining`, `maxVisitsPerDay`, `price`, `planName`). A member may hold more than one subscription. Archived members and archived plans cannot be sold. List, get, create, freeze, unfreeze, renew, and cancel return nested `member` (name, phone, status), `staff` who sold it (`id`, name, role — `null` on rows sold before that phase), `cancelledBy` (`id`, name, role — `null` until cancelled), current `plan` (name, status), `branches`, plus `paidTotal` and `dueAmount`. That seller is not edited later; renew is a **new** row with the staff who renewed. Desk lists: `unpaid=true` (zero or partial payment), `inProgress=true` (still running: live status and pack sessions left), `completedUnrenewed=true` (expired or pack used up, and they have not bought that plan again), `expired=true` (status `EXPIRED`, including rows that were later renewed). Combine with `memberId` / `planId` / `soldByStaffId` / `cancelledByStaffId`.
