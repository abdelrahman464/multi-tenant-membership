...
**Phase 26 (current):** date exceptions on branch hours. A 24/7 or weekly location can close (or use special hours) on a gym calendar day. The door uses that override. Payments still collect.
...
| GET | `/api/v1/subscriptions` | Any staff (`memberId`, `planId`, `soldByStaffId`, `cancelledByStaffId`, `status`, `unpaid`, `inProgress`, `completedUnrenewed`, `expired`, `endingSoon`, `sort`, `page`, `limit`) |
...
| GET | `/api/v1/reports/subscriptions` | Any staff (CSV; `memberId`, `planId`, `soldByStaffId`, `cancelledByStaffId`, `status`, `unpaid`, `inProgress`, `completedUnrenewed`, `expired`, `endingSoon`) |
