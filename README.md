...
**Phase 17 (current):** who sold the plan. Subscribe and renew store the staff at the desk. Old rows stay empty (`staff` is `null`).
...
| GET | `/api/v1/subscriptions` | Any staff (`memberId`, `planId`, `soldByStaffId`, `status`, `unpaid`, `inProgress`, `completedUnrenewed`, `expired`, `sort`, `page`, `limit`) |
...
| GET | `/api/v1/reports/subscriptions` | Any staff (CSV; `memberId`, `planId`, `soldByStaffId`, `status`, `unpaid`, `inProgress`, `completedUnrenewed`, `expired`) |
...
Subscribing snapshots the sold terms (`durationDays`, `sessionCount`, `sessionsRemaining`, `maxVisitsPerDay`, `price`, `planName`). A member may hold more than one subscription. Archived members and archived plans cannot be sold. List, get, create, freeze, unfreeze, and renew return nested `member` (name, phone, status), `staff` who sold it (`id`, name, role — `null` on rows sold before this phase), current `plan` (name, status), `branches`, plus `paidTotal` and `dueAmount`. That seller is not edited later; renew is a **new** row with the staff who renewed. Desk lists: `unpaid=true` (zero or partial payment), `inProgress=true` (still running: live status and pack sessions left), `completedUnrenewed=true` (expired or pack used up, and they have not bought that plan again), `expired=true` (status `EXPIRED`, including rows that were later renewed). Combine with `memberId` / `planId` / `soldByStaffId`.
